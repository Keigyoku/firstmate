...
- **Guarded by construction** - the first mate is read-only over your projects outside guarded clone refreshes, safe branch pruning, and approved `local-only` fast-forward merges; crewmates make every project change behind your merge approval, and crew/scout spawns get a process-signal guard against broad kills.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
  ...
  treehouse worktree, Orca worktree, or isolated secondmate home
     ...
     │
     └─ scout: report ► decision inventory ► relay findings ► teardown
```
...
| Skill              | What it does                                                                                                                                  |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `/afk`             | Enter away-mode supervision after verifying the injection channel: the sub-supervisor self-handles routine wakes in bash, escalates captain-relevant events and bounded declared-external-wait rechecks as batched digests, and actively alerts if delivery wedges |
...
- `.agents/skills/` - agent-loaded skills (this section's table, plus firstmate's agent-only reference skills, plus a few vendored third-party pins). Firstmate-authored entries assume a live firstmate home and are meaningless, or actively misleading, installed anywhere else, so each carries `metadata.internal: true` in its frontmatter. That flag hides them from installer discovery (tools like the [skills.sh](https://skills.sh) `npx skills add` installer) without affecting how firstmate itself loads them - frontmatter metadata is inert to the agent's own skill loader. Vendored third-party pins may keep upstream frontmatter byte-identical and document installer treatment in a sibling `PROVENANCE.md` instead of injecting that key.
...
- [docs/watchdog-w2-live-evidence.md](docs/watchdog-w2-live-evidence.md) - live validation evidence for the watchdog compact-steer flow.
- [docs/watchdog-w3-live-evidence.md](docs/watchdog-w3-live-evidence.md) - live validation evidence for watchdog successor handoff, clear rotation, and halt behavior.
