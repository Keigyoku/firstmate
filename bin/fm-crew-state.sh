...
#      exception: a declared `paused:` status outranks a DONE terminal run-step
#      (checks-green / completed) so a stand-by after green is not read as a
#      finished crew; FAILED/CANCELLED run-steps are never masked by a stale
#      pause (captain radar). Active working and parked runs still outrank pause.
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  # or captain merge (`paused: ...`). FAILED/CANCELLED must never be masked by a
  # stale pause (captain: do not let failures pass under the radar). An ACTIVE
  # working or parked run still wins so a crew that resumed after pausing is
  # never mis-classed as paused. See crew_absorb_class for the absorb half.
  if status_is_paused "$LOG_LINE" \
     && [ "$RUN_STATE" != working ] \
     && [ "$RUN_STATE" != parked ] \
     && [ "$RUN_STATE" != failed ]; then
