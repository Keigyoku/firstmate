...
# it only consults the shared read-only gate when present.
# Fork adaptation: fm-afk-return.sh is not ported yet; skip the gate when absent.
if [ -x "$SCRIPT_DIR/fm-afk-return.sh" ]; then
  ...
  | ([ .backlog.records[]
     ...
     + [ .tasks[]
         ...
         | select(
             ...
             or (
               ((.hints.last_event_text // "") | test("^needs-decision\\b"))
               and (.current_state.state != "done")
               and (.current_state.state != "failed")
               and (.current_state.state != "cancelled")
               and (.current_state.state != "working")
             )
           )
         | {id,key:.id,verb:"needs-decision",
            summary:((.hints.last_event_text // "needs decision") | trunc(90)),owner:"(main)"} ]
     + [ .tasks[]
         | select(.kind != "secondmate")
         | select(
             .hints.blocked_event == true
             or (
               ((.hints.last_event_text // "") | test("^blocked\\b"))
               and (.current_state.state != "done")
               and (.current_state.state != "failed")
               and (.current_state.state != "cancelled")
               and (.current_state.state != "working")
