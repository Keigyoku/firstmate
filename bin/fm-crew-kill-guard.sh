...
if [[ $flat =~ (^|[[:space:]/])(bash|sh|zsh|dash|ksh)[[:space:]][^\;\&\|]*-[^[:space:]]*c[[:space:]][^\;\&\|]*([^[:space:]]*/)?kill([[:space:]]|$) ]] ||
   ...
   [[ $flat =~ (^|[\;\&\|\(][[:space:]]*|[[:space:]](then|do)[[:space:]]+)(command[[:space:]]+|sudo[[:space:]]+([^[:space:]]+[[:space:]]+)*)?(([^[:space:]]*/)?(time|nohup|setsid|timeout|nice|ionice|chrt|stdbuf|unbuffer)([[:space:]]+[^[:space:]\;\&\|]+)*)[[:space:]]+([^[:space:]]*/)?kill([[:space:]]|$) ]] ||
