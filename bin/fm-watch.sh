...
# working signal is never silently swallowed. A deliberate hold is the separate
# idle absorb case: state/<id>.parked (written by bin/fm-park.sh) is the one
# representable hold fact; marker present absorbs idle-pane wedge noise and
# re-surfaces on the marker's cadence. Captain-relevant status signals still
# wake through the signal path regardless of the marker.
...
#                          firstmate hands it to a no-mistakes validation. A deliberate
#                          hold is absorbed instead with its own long re-surface cadence,
#                          never as a wedge. Only when neither
...
# grok: "Esc:cancel" (mid-turn keybind bar, re-verified grok 0.2.111 on 2026-07-23;
# keep Ctrl+c:cancel for older panes; ASCII avoids braille spinner fragility);
...
BUSY_REGEX=${FM_BUSY_REGEX:-'esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|Esc:cancel|ctrl\+c to stop|Ctrl\+C cancel'}
...
# A crew with state/<id>.parked (bin/fm-park.sh) is a deliberate hold: its stale
# pane is absorbed rather than wedge-escalated, and re-surfaces once per the
# marker's recheck_secs (default PAUSE_RESURFACE_SECS) so a forgotten hold cannot
# rot invisibly. Marker presence alone is the absorb fact - no run-step shape or
# status-verb corroboration.
...
watchdog_resident_rotation_active() {
  fm_watchdog_rotation_active "$1"
}

watchdog_resident_rotation_claim() {
  fm_watchdog_rotation_claim "$1" "$2"
}
...
watchdog_start_compact_wrap() {
  local task=$1 context=$2 compact=$3 sig=$4 sid=$5 key=$6 pending=$7 inflight=$8 generation=$9 pid claim request ack ack_q ack_tmp_q request_q message
  request="$(date -u +%s)-${BASHPID:-$$}-${RANDOM:-0}"
  ack="$STATE/watchdog/.compact-wrap-ack-$key"
  printf -v ack_q '%q' "$ack"
  printf -v ack_tmp_q '%q' "$ack.tmp.$request"
  printf -v request_q '%q' "$request"
  message="WATCHDOG WRAP REQUEST $request: Complete and land the current unit of work. Do not run /compact yourself. When wrapping is complete, acknowledge this exact request atomically by running: tmp=$ack_tmp_q; printf '%s\\n' $request_q > \"\$tmp\" && mv -f \"\$tmp\" $ack_q"
...
watchdog_threshold_scan() {
  local config compact successor embargo_5hr embargo_7d pending_retry wrap_ack_timeout failure_interval thresholds_enabled meta task harness metrics context five seven five_reset seven_reset file sig sid generation key handled pending inflight clear_pending clear_inflight err_file embargo_reason unsupported_marker phase
  ...
  [ -n "$compact$successor$embargo_5hr$embargo_7d" ] || thresholds_enabled=false
  ...
  for meta in "$STATE"/*.meta; do
    ...
    [ "$thresholds_enabled" = true ] || continue
...
# Absorb a stale pane whose crew has state/<id>.parked (bin/fm-park.sh).
...
# Re-check deliberate-hold classification. Marker present (or recent pause
# tracking with park still set) -> paused; else delegate to crew_absorb_class.
...
EOF
  ...
  while IFS= read -r w; do
    ...
    key=${key//\//_}
    key=${key//./_}
    if ! crew_is_parked "$task" && [ -e "$STATE/.paused-$key" ]; then
      clear_pause_tracking "$w"
    fi
    # kind=secondmate is a persistent idle endpoint, not a parked crew: skip stale
    # wedge detection unless firstmate explicitly parked this secondmate.
    if [ "$kind" = secondmate ] && ! crew_is_parked "$task"; then
      continue
    fi
    ...
    pf="$STATE/.paused-$key"   # flag: this key's current stale is a deliberate hold
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && ! window_is_busy "$w" "$tail40"; then
        ...
        if [ "$kind" = secondmate ]; then
          ...
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            ...
            case "$(pause_state_class "$w" "$task")" in
              ...
              working) clear_pause_state "$w"
                       printf '%s' "$h" > "$sf"
                       wedge_timer_check "$w" "$ssf" "stale (provably working after a deliberate hold)" "$ewf" ;;
          ...
          # on first sight, never every poll) via crew_absorb_class:
          #   - working: an actively-running pipeline legitimately sits on a static
          #     pane (e.g. waiting on CI), so absorb and start the wedge timer so a
          #     genuinely frozen run still escalates past STALE_ESCALATE_SECS;
          #   - paused: state/<id>.parked is present, so absorb on the marker's
          #     recheck cadence instead of wedge-escalating;
          #   - none: no running pipeline, idle pane, no park marker - the crew has
          #     STOPPED. Surface immediately so firstmate peeks.
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            task=$(window_to_task "$w" "$STATE")
            case "$(crew_absorb_class "$task")" in
              working)
            ...
            task=$(window_to_task "$w" "$STATE")
            if [ -e "$pf" ] || crew_is_parked "$task"; then
              case "$(pause_state_class "$w" "$task")" in
                paused)  handle_paused_stale "$w" "$task" "$h" ;;
                working) clear_pause_state "$w"
                         printf '%s' "$h" > "$sf"
                         wedge_timer_check "$w" "$ssf" "non-terminal stale (provably working after a deliberate hold)" "$ewf"
                         triage_log "absorbed non-terminal stale (provably working): $w" ;;
                *)       surface_nonterminal_stale "$w" "$h" ;;
              esac
            else
        ...
        rm -f "$ssf" "$ewf"
        task=$(window_to_task "$w" "$STATE")
        if [ -e "$pf" ] && { [ "$n" -ge 2 ] || ! crew_is_parked "$task"; }; then
          clear_pause_tracking "$w"
        fi
      fi
    else
      ...
      rm -f "$ssf" "$ewf"
      task=$(window_to_task "$w" "$STATE")
      if ! afk_present && crew_is_parked "$task" && ! window_is_busy "$w" "$tail40"; then
        case "$(pause_state_class "$w" "$task")" in
          paused) handle_paused_stale "$w" "$task" "$h" ;;
          *)      clear_pause_tracking "$w" ;;
        esac
