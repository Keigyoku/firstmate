...
# exempt (see the scoping block and docs/turnend-guard.md).
#
...
# worktree spawned to work on firstmate itself. A secondmate home runs its OWN
# primary firstmate session, so it must be guarded like the main primary; only
# child crew/scout worktrees are exempt. Physical-identity watcher matching and
# the settled healthy predicate are fork private evolution and must not regress.
#
...
# force-INCLUDE it (treehouse-leased linked or git-cloned plain). Only unmarked
# linked task worktrees stay exempt. Shared predicates live in
# bin/fm-primary-scope-lib.sh (one owner with subagent/cd-adjacent scope checks).
