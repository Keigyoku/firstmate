...
- **Work that spawns a real, longer-running job** (dispatching a crewmate, a scout investigation, a ship task) cannot report an outcome yet, so it follows **acknowledge first -> act -> follow up on completion**:
  ...
  3. **Link it for the follow-up, before clearing the inbox.** Associate the spawned task with this mention so completion follow-ups can be posted later: `bin/fm-x-link.sh <task-id> <request_id>` (records the request id, a timestamp, a follow-up counter, and reply platform/budget context).
     ...
     If a recovery respawns the same relay request onto a successor task, relink with the paired `--carry-count <n> --carry-ts <epoch>` flags plus the prior `x_platform=` as `--carry-platform <x|discord>` and, when present, `x_reply_max_chars=` as `--carry-max <n>` so the successor keeps the consumed follow-up count, original 7-day window, and reply split budget.
...
2. **Drain every pending mention.** For each `state/x-inbox/*.json` file:
   ...
   c. **Act on an actionable request through the normal lifecycle.** Treat it exactly as a captain prompt typed in session: run ordinary intake (resolve the project), then file the backlog item, dispatch a crewmate, start a scout, or ship through the gate - whatever the request calls for.
      ...
      **Link here, in step 2c, before the step 2f inbox cleanup** - `bin/fm-x-link.sh` can copy the mention's reply platform and any explicit budget from the still-present inbox payload without a relay lookup.
      If the platform is missing it uses the durable resolution contract in `docs/configuration.md` and warns loudly, while an omitted optional budget uses the resolved platform's configured default.
...
- If a linked task is replaced by a successor for the same relay request, carry the prior `x_followups=`, `x_request_ts=`, and `x_platform=` values with `bin/fm-x-link.sh <new-task-id> <request_id> --carry-count <n> --carry-ts <epoch> --carry-platform <x|discord>`, adding `--carry-max <n>` when `x_reply_max_chars=` is present, so recovery preserves the consumed follow-up count, original window, and reply split budget after the inbox file is gone.
...
- If a follow-up's platform cannot be authoritatively resolved from per-request context, inbox payload, or relay answer, `bin/fm-x-followup.sh` does NOT post it: the fail-safe holds it (the link is kept, exit non-zero) rather than guess a platform.
  This is a retryable hold - a later milestone wake retries it once the platform is recoverable, while a known platform with no explicit budget uses its configured default.
