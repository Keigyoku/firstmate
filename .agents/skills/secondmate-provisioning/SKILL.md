...
`bin/fm-spawn.sh --secondmate` launches it through the secondmate harness path, resolving `config/secondmate-harness` first and then the active crew harness, including watchdog rotation, unless an explicit per-spawn harness override is passed.
