...
.agents/skills/      firstmate-loaded internal skills, committed; firstmate-authored entries carry metadata.internal=true for installers (vendored third-party pins may keep upstream frontmatter and record installer treatment in PROVENANCE.md)
...
config/claim-guard  optional primary-session claim-vs-evidence coaching disable; LOCAL, gitignored; exactly `off` disables the non-blocking claim reminder, absent or any other value leaves it enabled (docs/turnend-guard.md)
...
fm-state/            local operational event logs; gitignored
  ...
  last-glass-capture freshness marker written by bin/fm-glass.sh ("epoch path"); read by bin/fm-claim-guard.sh as the receipt for rendered-app claims (docs/turnend-guard.md)
...
Running, fixing, or CI states remain working; parked approval or fix-review states require the worker to follow the active gate help; passed or checks-passed is done; failed is failed; cancelled is its own state, because releasing branch custody after a green run is the sanctioned workflow rather than an alarm.
...
The claim-vs-evidence coaching reminder (`bin/fm-claim-guard.sh` records on Stop, `bin/fm-claim-coach-inject.sh` delivers on the next prompt) rides the same primary-session path but NEVER blocks a turn; exact `off` in `config/claim-guard` disables it.
