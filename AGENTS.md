...
Do not bulk-read `data/backlog.md` afterward either: the compact identity/metadata listing was just printed (owned by `bin/fm-session-start.sh`); use `tasks-axi show <id> --full` or a targeted file read only when a full body is needed.
...
- `WATCHDOG: invalid config/watchdog.json - malformed JSON; using defaults` - bootstrap could parse neither the optional local watchdog config nor any useful defaults from it; leave it in place only if the captain intends to fix it, otherwise remove it to use defaults.
...
- `TASKS_AXI: available` - a default-backend capability fact, not a problem; record it silently and use section 10 for backlog mutations.
  ...
  If the backend is not opted out and `tasks-axi` is missing or incompatible, bootstrap reports `MISSING: tasks-axi (install: npm install -g tasks-axi)` but firstmate still hand-edits routine backlog updates and never blocks work.
  If `config/backlog-backend=manual`, bootstrap hand-edits routine backlog updates and does not suggest installing `tasks-axi`.
...
If `default` is absent, fall back to the active crew harness through `bin/fm-harness.sh crew`, including watchdog rotation, but still pass that resolved harness explicitly.
...
The requirement is gated only on the file's presence; when the file is absent, `fm-spawn.sh` keeps resolving the active crewmate harness through `bin/fm-harness.sh crew`.
...
`config/secondmate-harness` (local, gitignored) is the harness the primary uses to launch SECONDMATE agents; resolve it with `bin/fm-harness.sh secondmate`, which reads `config/secondmate-harness` first and otherwise falls through the active crew harness.
So an absent or `default` `config/secondmate-harness` behaves exactly as before this knob existed - secondmates launch on the crew harness, including watchdog rotation - and setting it splits the two: e.g. primary `config/crew-harness=codex` with `config/secondmate-harness=claude` runs the secondmate AGENTS on claude while all crewmates (the primary's and the secondmates' own) run on codex.
...
3. The digest's fleet-state section already printed the compact backlog listing (see section 3), every `state/*.meta`, and a bounded tail of every `state/*.status` (`data/secondmates.md` came from the context section).
...
For role-identity dispatches, pass matching explicit `--role` flags to both `fm-brief.sh` and `fm-spawn.sh`; never infer a role from the task id, and use each script's `--help` for the allowed roles and delivery mechanics.
...
In all cases, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), re-spells treehouse worktree paths through `$HOME` when that spelling names the same physical directory, installs the per-task turn-end signal hook, crew/scout kill guard, and ship-crew TDD pre-execution guard (docs/crew-tdd-guard.md) where applicable, records `state/<id>.meta`, and launches the agent with the brief.
Selecting `backend=codex-app` fails as an unknown backend; see `docs/codex-app-backend.md`.
For grok, turn-end, kill-guard, and (for ship crews, unless TDD is hatched off) TDD pre-execution guard hooks are firstmate-owned global hooks under `$GROK_HOME/hooks/`, or `~/.grok/hooks/` when `GROK_HOME` is unset, activated only when the worktree holds the matching per-task pointer; teardown removes each pointer and token.
...
If `treehouse return` says a treehouse-backed worktree is not managed, teardown retries a physically equivalent `$HOME` spelling before failing; this covers hosts where `$HOME` and `pwd -P` spell the same home differently.
If treehouse surfaces Git's "Another git process seems to be running" lock message, teardown makes one bounded retry without forcing or deleting the lock, then uses the existing provably-stale lock cleanup only when the lock still meets that proof.
...
The watcher classifies every wake it detects in bash and absorbs the benign majority without ever waking you, but it never absorbs an unmarked crewmate that has stopped.
...
A crewmate that declares a deliberate external wait with a `paused:` status is the one other absorb case: its idle pane is expected, so the watcher absorbs it like a working crew but rechecks it only on the long `FM_PAUSE_RESURFACE_SECS` cadence (default 3600s) rather than the wedge threshold, re-surfacing a `paused ... recheck` wake once per window so a forgotten pause cannot rot invisibly - unlike `blocked:`, which stays a genuine stuck-crew escalation.
After relaying a verified ask-user decision to the captain from any wake path, run `bin/fm-held-gate-mark.sh <id>` so the firstmate-owned marker enables that same bounded hold mechanism; the lifecycle is owned by `docs/architecture.md` "Event-driven supervision".
In always-on mode the initial `paused:` status still surfaces once through the no-verb path, while the away-mode daemon self-handles that routine signal and owns the later recheck.
...
The classifier lives in `bin/fm-classify-lib.sh` and is shared: the captain-relevant verb set, the `paused:` external-wait verb and its re-surface cadence, and the status-scan primitives back both this always-on watcher and the away-mode daemon, so the overlapping policy cannot drift; the provably-working and paused predicates (`crew_absorb_class`, reusing `bin/fm-crew-state.sh`) live in that same library and run only on the watcher's no-verb signal and first-sighting stale paths, never on every wake, so the per-wake triage stays cheap.
...
Firstmate's primary checkout (the repo root, `FM_ROOT`) and crewmate worktrees share the treehouse pool, while a secondmate home may be a linked worktree or a standalone clone, and the primary must stay on its default branch.
...
- `decision-hold-lifecycle` - load before treating an investigation, scout report, structured review, or visual review as complete, and when recording or routing the captain's answer.
