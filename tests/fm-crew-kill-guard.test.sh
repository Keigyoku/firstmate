...
expect_deny '2>&1 pkill -f app'
expect_deny '>&2 kill -9 -1'
expect_deny '>|/tmp/out killall app'
expect_deny '&>/tmp/out pkill -f app'
expect_deny '&>>/tmp/out pkill -f app'
...
expect_deny 'sudo -H pkill -f app'
expect_deny 'sudo --askpass pkill -f app'
expect_deny 'sudo FOO=bar pkill -f app'
expect_deny 'sudo FOO=bar kill -9 -1'
...
expect_deny 'p[k]ill -f app'
expect_deny 'k[i]ll -9 -1'
expect_deny '?kill -f app'
expect_deny 'kil* -9 -1'
...
expect_allow 'sudo -H kill 123'
expect_allow 'sudo --askpass kill 123'
expect_allow 'sudo FOO=bar kill 123'
