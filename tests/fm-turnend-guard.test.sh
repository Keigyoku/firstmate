...
run_hook() {
  ...
  home=$(cd "$dir" && pwd)
  printf '{"stop_hook_active":%s}' "$stop_active" | CLAUDECODE=1 CLAUDE_PROJECT_DIR="$home" FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" 2>&1
}

run_hook_without_project_dir() {
  local dir=$1 stop_active=$2
  printf '{"stop_hook_active":%s}' "$stop_active" | (
    cd "$dir" && env -u CLAUDE_PROJECT_DIR -u FM_HOME -u FM_ROOT_OVERRIDE -u FM_STATE_OVERRIDE CLAUDECODE=1 bash bin/fm-turnend-guard.sh
  ) 2>&1
}
...
test_hook_blocks_from_fm_home_state() {
  ...
  out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 CLAUDE_PROJECT_DIR="$dir" FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" 2>&1); status=$?
...
test_hook_uses_state_override() {
  ...
  : > "$state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 CLAUDE_PROJECT_DIR="$dir" FM_HOME="$home" FM_STATE_OVERRIDE="$state" bash "$dir/bin/fm-turnend-guard.sh" 2>&1); status=$?
...
# exactly like the main primary (upstream #505).
...
# continuation per turn, exactly as it does for the main primary.
...
# Deferred-death recovery loop in a secondmate home.
