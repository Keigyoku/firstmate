...
test_secondmate_registry_contract_stays_concise() {
  local routing_section schema_line
  routing_section=$(awk '
    /^## Routing table$/ { found = 1 }
    found && /^## Charter and seed$/ { exit }
    found { print }
  ' "$SECONDMATE")
  schema_line="- <id> - <charter summary> (home: <absolute-home-path>; scope: <natural-language responsibility>; projects: <project-a>, <project-b>; added <date>)"
  assert_contains "$routing_section" "$schema_line" \
    "secondmate routing table lost the parser-compatible single-line schema"
  assert_contains "$routing_section" "The \`scope:\` field is used during intake." \
    "secondmate routing table lost the intake-scope rule"
  assert_contains "$routing_section" "non-exclusive clone list, not ownership" \
    "secondmate routing table lost the non-ownership rule for projects"
  assert_grep "idle-by-default contract" "$SECONDMATE" \
    "secondmate provisioning lost the idle-by-default charter contract"
  assert_grep "marked supervisor requests return through status" "$SECONDMATE" \
    "secondmate provisioning lost the marked-return-channel contract"
  assert_grep "unmarked captain messages stay conversational" "$SECONDMATE" \
    "secondmate provisioning lost the unmarked-captain conversational rule"
  pass "secondmate registry guidance keeps concise routes and charter ownership"
}

test_state_startup_and_ordinary_recovery_placement() {
  assert_grep "single owner of the top-level operational-home layout" "$AGENTS" \
    "AGENTS.md does not point layout ownership at docs/configuration.md"
  assert_grep "header is the single owner of composed commands, ordering, and digest contents" "$AGENTS" \
    "AGENTS.md does not point session-start ownership at the script header"
  ...
  assert_grep "load \`bootstrap-diagnostics\`" "$AGENTS" \
    "AGENTS.md lost the bootstrap-diagnostics load trigger"
...
test_compressed_agents_retains_authority_and_supervision_safety() {
  ...
  # local-only mode intentionally has firstmate review the diff before captain merge;
  # the no-mistakes path must not invent a stacked personal clean gate.
  if grep -n 'firstmate reviews your branch' "$BRIEF" | grep -vi 'local'; then
    fail "generated brief retained a stacked personal-review requirement outside local-only"
  fi
