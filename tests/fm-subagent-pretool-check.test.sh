...
test_claude_hook_registration_preserves_bash_seatbelts() {
  ...
  # Fork composition at B1: Bash rail holds arm-pretool only. Later batches may
  # add cd/continuity on the same matcher; this assertion requires arm-pretool
  # so the subagent wiring cannot replace or shadow that private seatbelt.
  jq -e '
