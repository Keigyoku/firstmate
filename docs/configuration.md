...
When bootstrap resolves `backend=orca` from `FM_BACKEND` or `config/backend`, it checks that `orca --help` exposes the expected Orca developer CLI commands (`status`, `repo`, `worktree`, and `terminal`), keeps the universal `node` requirement, and skips the tmux/treehouse tool pair because Orca owns both the worktree and terminal lifecycle.
...
Zellij has no per-home workspace split: primary and secondmate tasks share that one session, task selectors can be exact ids or stable `fm-<id>` labels, and visible tab titles are scoped by the active `FM_HOME` readable label plus a short hash of the resolved `FM_ROOT` path as `fm-<home-label>-<id>`.
...
The active fields are `poll_interval_sec`, `thresholds.compact_at_context_pct`, `thresholds.successor_at_context_pct`, `thresholds.embargo_at_5hr_pct`, `thresholds.embargo_at_7d_pct`, `steer_retries`, `steer_timeout_sec`, `compact_pending_retry_sec`, `metrics_failure_event_interval_sec`, and `parser_version`.
`reserved_w4.rotate_to` in the example is reserved for W4 budget rotation and is ignored by W3 successor spawning, which reuses the predecessor harness.
...
Fresh links start with `x_followups=0` and the current timestamp; when relinking the same relay request onto a successor task, pass paired `--carry-count <n> --carry-ts <epoch>` flags so the successor preserves the already-consumed follow-up count and original 7-day window.
...
FM_BACKEND_HERDR_IDLE_RE='^Type a message\.\.\.$'  # herdr-only: empty-composer placeholder regex after ANSI, border, and prompt stripping
...
FMX_FOLLOWUP_MAX_AGE_SECS=604800   # local window for posting X completion follow-ups (7 days)
FMX_FOLLOWUP_MAX_COUNT=3   # local cap on X completion follow-ups per linked mention
