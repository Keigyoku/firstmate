...
When away-mode activation cannot verify the injection channel, the sub-supervisor fails immediately and raises a loud alarm.
If later injection wedges past `FM_MAX_DEFER_SECS`, it raises the same alarm on a rate-limited cadence.
Beyond the durable `state/.subsuper-inject-wedged` marker and the tmux status-line flash, either path attempts a configured backend-independent active alert that can reach the captain even when every pane and its backend status-line is unreadable.
...
An absent file means `auto`, i.e. default-on on macOS: an activation failure fires immediately, while a later delivery wedge fires at most once per max-defer window.
...
Teardown of a leased home fails closed if `treehouse return` cannot release the lease; on a "not managed" failure it first retries a physically equivalent `$HOME` spelling, and plain-clone homes with no treehouse pool slot are removed directly.
...
Crew and scout TDD pre-execution guard integrations ride those same rails for claude/codex/opencode/pi/grok and are documented in [`docs/crew-tdd-guard.md`](crew-tdd-guard.md); cursor and hermes stay outer-gate-only for TDD.
...
The local, gitignored `config/tdd-hook` file disables the crew TDD pre-execution guard only when its content is exactly `off`; `FM_TDD_HOOK_OFF=1` is the env equivalent (temporary until tuned).
...
For grok, `fm-spawn.sh` installs firstmate-owned global turn-end, kill-guard, and (when TDD is not hatched off) tdd-guard hooks under `$GROK_HOME/hooks/`, or `~/.grok/hooks/` when `GROK_HOME` is unset, and drops per-task pointer files in the worktree, with teardown removing each task token and pointer.
...
`use` may be one profile object or a non-empty ordered array of profile objects, and every profile requires `harness`.
`model`, `effort`, `why`, `select`, and `default` are optional.
An omitted model or effort uses the selected harness's default for that axis.
`select` currently accepts only `quota-balanced`; when it is absent, the first array element or sole object is selected.
`bin/fm-dispatch-select.sh` owns the deterministic quota-balanced selection and fallback mechanics.
...
If that attempt returns ambiguously before rotation is visible, the watcher preserves the pending identity and generation proof without retrying, then either marks a later rotation handled or enters successor handoff after another `compact_wrap_ack_timeout_sec` while retaining the proof through a skipped or failed handoff.
After a successful `/compact` send, the watcher likewise preserves the identity and generation proof without retrying, then either marks the resulting rotation handled or enters successor handoff after `compact_wrap_ack_timeout_sec` while retaining the proof until rotation.
...
Ordinary steers are retried up to `steer_retries`, each delivery is bounded by `steer_timeout_sec`, and a pending clear is retried only after `compact_pending_retry_sec` unless Claude/Codex transcript rotation proves a new session has taken over.
...
In that case the link is still recorded but `bin/fm-x-link.sh` prints a loud warning; and when a follow-up's platform cannot be authoritatively resolved from any source, `bin/fm-x-reply.sh` refuses it (fail-safe exit 8) rather than guessing a platform - firstmate holds and retries it once the platform is recoverable, while a resolved platform with no explicit budget safely uses that platform's configured default.
...
It treats `fm-x-reply.sh`'s fail-safe refusal (exit 8: platform unresolved) differently: that is a retryable hold, so the link is KEPT and the follow-up is retried once the platform can be recovered, never posted against a guessed platform.
...
FM_BACKEND_HERDR_INPUT_SETTLE=0.2  # herdr-only: seconds to wait before pane input calls after target readiness
...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/parked watcher triage
...
FM_PROC_ROOT_OVERRIDE=   # test-only alternate proc root for Linux watcher process identity; unset uses /proc
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, and the primary turn-end guard treat a watcher beacon as stale
FM_TURNEND_LOCK_SETTLE=1   # seconds the primary turn-end guard may wait for a live, fresh, newly published watcher lock to gain identity fields
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; an unmarked stale pane whose crew is not provably working surfaces immediately
FM_PAUSE_RESURFACE_SECS=3600       # default recheck_secs written for a deliberate hold; each state/<id>.parked marker owns its actual watcher/daemon cadence
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|Esc:cancel|ctrl\+c to stop|Ctrl\+C cancel'   # busy-pane signatures, shared by watcher, fm-crew-state pane fallback, and tmux helper
