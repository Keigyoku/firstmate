...
That path acquires the session lock with the start process PID (`FM_LOCK_PID`), runs setup and publishes `ready` with `FM_RESIDENT_HARNESS` (basename of `<harness>` when unset), then `exec`s the harness so the same PID becomes the agent image. Ancestry walks from children of the harness still find a real harness process; there is no leftover start shell after exec.
