...
When `fm-send.sh` has already scoped a target as cursor-agent mid-turn, cmux accepts the same optional queue-push flag as the other submit-verifying backends and sends one additional Enter after the verified composer clear.
