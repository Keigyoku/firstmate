...
| Send + submit, composed | `send` then `send-key enter` | cmux has no single-call atomic "type and submit" primitive; `fm_backend_cmux_send_text_line` composes the two calls, mirroring zellij's equivalent composition. |
...
| Kill | `cmux close-workspace --workspace <id>`, preceded by a throwaway `new-workspace --window <win> --focus false --id-format uuids` when the target is the only workspace in its window | See "Closing the last workspace in a window" below. The backend owns the whole task workspace; kill scans every window to validate or recover the expected task label, fails closed so teardown preserves metadata when that scan or sibling creation is incomplete, and treats only the final `close-workspace` call as best-effort. |
...
The next section ("Closing the last workspace in a window") owns the last-in-window exception and `fm_backend_cmux_kill`'s guarded workaround, which still reclaims every surface in the task workspace.
...
Expected-label teardown uses the same all-window scan to validate a current workspace id or recover a stale one by its unique home-scoped title, so a task parked outside the current window remains safely removable.
If any window or surface scan is incomplete, the title match is ambiguous, or the required sibling cannot be created, kill returns failure and `fm-teardown.sh` preserves the task metadata for a later retry instead of treating unknown state as already cleaned up.
...
The same unit suite pins cross-window stale-id recovery, ambiguous or incomplete scan refusal, sibling-creation refusal, and metadata preservation through direct and nested secondmate teardown.
...
Teardown's window lookup and expected-label recovery pass `--window` per window and are unaffected, but `fm_backend_cmux_workspace_id_for_label` and `fm_backend_cmux_list_live` see only the current window's workspaces, and ordinary `fm_backend_cmux_target_ready` label recovery inherits that scope.
This remains a known limitation for non-teardown label recovery and orphan discovery when a task workspace is parked in a non-current window.
...
When `fm-send.sh` has already scoped a target as cursor-agent or grok mid-turn, cmux accepts the same optional queue-push flag as the other submit-verifying backends and sends one additional Enter after the verified composer clear.
All implemented submit-verifying backends expose the identical caller-facing verdict vocabulary (`empty`, `pending`, `unknown`, `send-failed`), so `fm-send.sh` needs no cmux-specific submit-verdict branching.
...
- **Windows cannot be closed over the control socket, and ordinary label lookup is current-window scoped** - both owned by "Closing the last workspace in a window" above. Teardown of a last-in-window task workspace therefore leaves that window a fresh default workspace rather than closing it; teardown itself scans all windows, but `fm_backend_cmux_workspace_id_for_label`/`fm_backend_cmux_list_live` still see only the current window, so a task workspace parked in a non-current window remains a known blind spot for ordinary label recovery and orphan discovery.
