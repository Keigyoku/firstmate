...
```text
{"client":{"version":"0.7.3","channel":"stable","protocol":16,"binary":"/etc/profiles/per-user/<user>/bin/herdr","session":null},"server":{"status":"running","running":true,"version":"0.7.3","protocol":16,"capabilities":{"live_handoff":true,"detached_server_daemon":false},"compatible":true,"socket":"<home>/.config/herdr/herdr.sock","session":null,"restart_needed":false}}
codex-cli 0.142.1
```
...
- **RESOLVED: a representable deliberate-hold state for stale-wedge escalation.** A crew intentionally idling on a known external wait previously triggered the same possible-wedge escalation as a genuinely wedged crew.
  The backend-independent park-marker contract now owns this behavior; see [`architecture.md`](architecture.md#event-driven-supervision).
