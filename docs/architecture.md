...
The context-threshold compact handshake is specified in [configuration.md](configuration.md#watchdog-metrics).
...
`bin/fm-rotate-resident.sh` is the manual entry point for that same successor path: it resolves the live resident task from the current backend endpoint, refuses when the watchdog is halted or another rotation is in flight, and can print a non-mutating `--dry-run` plan before creating the handoff.
...
Pending clear steers are rate-limited by `compact_pending_retry_sec`, resident-rotation locks serialize manual and watchdog-initiated rotations for the same task, metrics-parser failures are throttled by `metrics_failure_event_interval_sec`, and malformed watchdog config falls back to defaults after a bootstrap `WATCHDOG:` diagnostic.
...
The lock step also provisions and publishes the Crew Lead resident current-state pointer, using the existing session-lock authority instead of a parallel primary-session tracker; the portable schema and versioning rules live in [crew-lead-resident-contract.md](crew-lead-resident-contract.md).
...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it starts the watcher orphaned in a separate process session with standard file descriptors detached from the arm task, keeps monitoring that process for wake delivery, verifies it is genuinely alive with a fresh liveness beacon, and prints exactly one honest status line (`started` / `attached` / restart-only `healthy` / `FAILED`, the last exiting non-zero).
The orphaned parentage, separate session, and detached descriptors keep a harness task-manager stop that discovers descendants, kills the arm's process group, or closes its pipes from also killing the confirmed watcher.
...
The guard is scoped out of secondmate homes and crewmate/scout worktrees, is loop-safe per harness, writes a size-capped diagnostic for each warning decision, and is documented in [turnend-guard.md](turnend-guard.md).
...
That keeps spawn launch compatible across claude, codex, grok, pi, opencode, cursor, and hermes while preserving the requested profile for later audit.
...
On the locked session-start bootstrap step, that token creates two local artifacts: `state/x-watch.check.sh`, which performs one bounded relay poll through `bin/fm-x-poll.sh`, and `config/x-mode.env`, which sets `FM_CHECK_INTERVAL=30` for watcher processes in that home.
...
The current watcher reliability work combines always-on bash triage with a durable queue for actionable wakes, a race-proof singleton lock, duplicate self-eviction, drain-time liveness assertion, and a self-verifying session-isolated arm wrapper.
