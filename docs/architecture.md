...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it starts the watcher in a separate process session with standard file descriptors detached from the arm task, keeps waiting on that child for wake delivery, verifies it is genuinely alive with a fresh liveness beacon, and prints exactly one honest status line (`started` / `attached` / restart-only `healthy` / `FAILED`, the last exiting non-zero).
The separate session and detached descriptors keep a harness task-manager stop of the arm's process group or pipes from also killing the confirmed watcher.
