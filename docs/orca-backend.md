...
It is distinct from the crewmate harness: the harness is the agent process firstmate launches (`claude`, `codex`, `opencode`, `pi`, `grok`, `cursor`, or `hermes`), while Orca owns the task worktree and terminal endpoint underneath that process.
...
- The `orca` CLI: `brew install orca`.
  Bootstrap and spawn both verify that `orca --help` looks like the expected Orca developer CLI by checking for the `status`, `repo`, `worktree`, and `terminal` command names, so a different executable named `orca` is treated as missing for this backend.
...
4. Install firstmate's per-harness turn-end hooks and the crew kill guard in the Orca worktree.
...
- `fm-send.sh` types text with `orca terminal send --text ...`, submits with Enter, and verifies the composer row cleared before returning; when Orca reports a limited page, the verifier follows `oldestCursor` and preserves the current tail so older text cannot hide still-pending composer input.
  ...
  When `fm-send.sh` has already scoped a target as cursor-agent or grok mid-turn, the Orca adapter accepts the same optional queue-push flag as the other submit-verifying backends and sends one additional Enter after the verified clear.
...
- Scout teardown still requires `data/<id>/report.md` and the shared `decision-hold-lifecycle` completion gate unless `--force` is explicitly used.
